document.addEventListener("DOMContentLoaded", async function () {
  const googleSheetURL =
    "https://docs.google.com/spreadsheets/d/1Qcvxk7tC9k7l3kjUlw5AgzuVb8aprbJvzrsGEvG4CMA/export?format=csv&gid=305762792";
  ...
  const detailsModalBodyContent = document.getElementById(
    "detailsModalBodyContent"
  );

  const projectListModalEl = document.getElementById("projectListModal");
  const projectListModal = projectListModalEl
    ? new bootstrap.Modal(projectListModalEl)
    : null;
  const projectListModalTitle = document.getElementById(
    "projectListModalTitle"
  );
  ...
  function populateTable(data) {
    ...
    data.forEach((school) => {
      ...
      const originalIndex = schoolData.findIndex(
        (originalSchool) => originalSchool === school
      );
      ...
      row.innerHTML = `
                ...
                <td data-label="Comprometido">${formatCurrency(
                  school.valorComprometido.total
                )}</td>
                <td data-label="Disponível" class="fw-bold ${
                  school.saldoDisponivel.total < 0 ? "text-danger" : ""
                }">${formatCurrency(school.saldoDisponivel.total)}</td>
                <td data-label="Status"><span class="status-badge ${statusClass}">${
        school.statusConta
      }</span></td>
                <td data-label="Ações"><button class="btn btn-details" data-index="${originalIndex}">Detalhes</button></td>
            `;
  ...
  function updateCards(data) {
    document.getElementById("totalSaldoInicial").textContent = formatCurrency(
      data.reduce((sum, item) => sum + item.saldoInicial.total, 0)
    );
    document.getElementById("totalSaldoDisponivel").textContent =
      formatCurrency(
        data.reduce((sum, item) => sum + item.saldoDisponivel.total, 0)
      );
    document.getElementById("totalComprometido").textContent = formatCurrency(
      data.reduce((sum, item) => sum + item.valorComprometido.total, 0)
    );
    document.getElementById("totalEscolas").textContent = data.length;
    document.getElementById("labsImplantados").textContent = `${
      data.filter(
        (item) => (item.labInformatica || "").toUpperCase() === "IMPLANTADO"
      ).length
    } Implantados`;
    document.getElementById("escritoriosImplantados").textContent = `${
      data.filter(
        (item) =>
          (item.escritoriosCriativos || "").toUpperCase() === "IMPLANTADO"
      ).length
    } Implantados`;
    document.getElementById("totalAgro").textContent = `${
      data.filter(
        (item) =>
          (item.projetoAgroecologico || "").toUpperCase() === "IMPLANTADO"
      ).length
    } Implantados`;
    document.getElementById("totalRobotica").textContent = `${
      data.filter(
        (item) => (item.labRobotica || "").toUpperCase() === "IMPLANTADO"
      ).length
    } Implantados`;
  }
  ...
  function populateFilters(data) {
    municipioFilter.innerHTML =
      '<option value="todos">Todos os Municípios</option>';
    const municipios = [
      ...new Set(data.map((school) => school.municipio).filter(Boolean)),
    ];
  ...
  function applySort(data) {
    data.sort((a, b) => {
      ...
      let valA, valB;
      if (
        ["saldoInicial", "valorComprometido", "saldoDisponivel"].includes(key)
      ) {
        valA = a[key].total;
        valB = b[key].total;
  ...
  function filterAndDisplayData() {
    ...
    currentlyDisplayedData = schoolData.filter((school) => {
      ...
      const matchesMunicipio =
        selectedMunicipio === "todos" || school.municipio === selectedMunicipio;
  ...
  // FUNÇÃO showDetailsModal ATUALIZADA PARA O LAYOUT ANTIGO
  function showDetailsModal(index) {
    ...
    else if (statusConta.includes("PROCESSO"))
      statusBadgeClass = "bg-warning-subtle text-warning-emphasis";

    detailsModalBodyContent.innerHTML = `
            <h4 class="mb-2" style="color: var(--dark-purple);">${
              school.escola || "Nome não informado"
            }</h4>
            <p class="text-muted mb-3">${
              school.municipio || "N/A"
            } | <strong></strong> ${school.nte || "N/A"}</p>
            <div class="school-codes">
                <div><p>CÓDIGO INEP (MEC)</p><span>${
                  school.inep || "N/A"
                }</span></div>
            ...
            <div class="modal-body-columns">
                <div class="detail-group">
                    ...
                    <div class="row g-3">
                        <div class="col-md-4">
                            ...
                            <div class="detail-item"><p>Capital:</p> <span>${formatCurrency(
                              school.saldoInicial.capital
                            )}</span></div>
                            <div class="detail-item"><p>Custeio:</p> <span>${formatCurrency(
                              school.saldoInicial.custeio
                            )}</span></div>
                            <div class="detail-item"><p>Total:</p> <span>${formatCurrency(
                              school.saldoInicial.total
                            )}</span></div>
                        </div>
                        <div class="col-md-4">
                            ...
                            <div class="detail-item"><p>Capital:</p> <span>${formatCurrency(
                              school.valorComprometido.capital
                            )}</span></div>
                            <div class="detail-item"><p>Custeio:</p> <span>${formatCurrency(
                              school.valorComprometido.custeio
                            )}</span></div>
                            <div class="detail-item"><p>Total:</p> <span>${formatCurrency(
                              school.valorComprometido.total
                            )}</span></div>
                        ...
                        <div class="col-md-4">
                            ...
                            <div class="detail-item"><p>Capital:</p> <span>${formatCurrency(
                              school.saldoDisponivel.capital
                            )}</span></div>
                            <div class="detail-item"><p>Custeio:</p> <span>${formatCurrency(
                              school.saldoDisponivel.custeio
                            )}</span></div>
                            <div class="detail-item"><p>Total:</p> <span>${formatCurrency(
                              school.saldoDisponivel.total
                            )}</span></div>
                        </div>
                    </div>
                </div>
                <div class="detail-group">
                    ...
                    <div class="detail-grid">
                        <div class="detail-item"><p>Lab. Informática:</p> <span>${
                          school.labInformatica || "N/A"
                        }</span></div>
                        <div class="detail-item"><p>Escritórios Criativos:</p> <span>${
                          school.escritoriosCriativos || "N/A"
                        }</span></div>
                        <div class="detail-item"><p>Proj. Agroecológico:</p> <span>${
                          school.projetoAgroecologico || "N/A"
                        }</span></div>
                        <div class="detail-item"><p>Lab. Robótica:</p> <span>${
                          school.labRobotica || "N/A"
                        }</span></div>
                    </div>
                </div>
            </div>
  ...
  async function initialize() {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(
          "Falha ao carregar dados. Verifique o link e o compartilhamento da planilha."
        );
